...
city <- bc_cities() %>%
 ...
 cities_wc <- cities 
   #dplyr::filter(NAME %in% cities_regions) if you want to filter for specific cities
...
- The flow category "High" indicates that the estimated streamflow is the <span style="color: #000000;">highest value</span> ever measured for the day of the year
- A streamflow which is greater than the 90th percentile is considered above <span style="color: #0003F6;">much above normal</span>
- A streamflow which is between  75th percentile and 90th percentile is considered  <span style="color: #46DED2;">above normal</span>
- A streamflow which is between 25th and 75th percentiles is considered <span style="color: #08FA09;">normal</span>
- A streamflow which is less than the 25 percentile is considered <span style="color: #FEA116;">below normal</span>
- A streamflow which is less than 10 percentile is considered <span style="color: #B31F23;">much below normal</span>
- The flow category "Low" indicates that the estimated streamflow is the <span style="color: #F90010;">lowest value</span> ever measured for the day of the year
...
if (params$table_format == "html") {
 ...
 # pal <- colorFactor(palette = naww_pal, domain = expected)
  ...
  leaflet() %>%
   ...
   addMapPane("Drought Polygons", zIndex = 405) %>%
   ...
   addPolygons(data = region_watershed_interactive,
              ...
              weight = 3,
              color = '#696969',
              fillColor = '#696969', fillOpacity = 0.3, 
...
The table below lists stations that are below their 10th percentile in terms of the seven day discharge data for today. If a location is not listed here there are no immediate low streamflow concerns. 
```{r belownormal}
much_below_normal_tbl <- as.data.frame(table_out) %>%
  dplyr::filter(pct_bin_7day %in% c("Low","Much below normal (<10)")) %>%
  ...
  dplyr::arrange(`Percentile - Q7`, `Percentile - Last 24 Hour Q`) 
                                                                         ...
                                                                         ifelse(`Percentile Category - Last 24 Hour Q` == "Normal (25-75)" , "#08FA09", 
                                                                                ifelse(`Percentile Category - Last 24 Hour Q` == "Above normal (76-90)" , "#46DED2", 
                                                                                       ifelse(`Percentile Category - Last 24 Hour Q` == "Much above normal (>90)" , "#0003F6",
                                                                                              "#000000")))))))) %>%
    dplyr::mutate(`Percentile Category - 7 Day Q` = cell_spec(`Percentile Category - 7 Day Q`, "html",
                                            color = ifelse(`Percentile Category - 7 Day Q`  == "Low", "#F90010",
                                                           ifelse(`Percentile Category - 7 Day Q` == "Much below normal (<10)", "#B31F23" ,
                                                                  ifelse(`Percentile Category - 7 Day Q` == "Below Normal (10-24)" , "#FEA116", 
                                                                         ifelse(`Percentile Category - 7 Day Q` == "Normal (25-75)" , "#08FA09", 
                                                                                ifelse(`Percentile Category - 7 Day Q` == "Above normal (76-90)" , "#46DED2", 
                                                                                       ifelse(`Percentile Category - 7 Day Q` == "Much above normal (>90)" , "#0003F6",
                                                                                              "#000000")))))))) %>%
...
### Table of Stations Below 10% LT MAD
Any stations with LT MAD values of less than or equal to <span style= "color: #B31F23;">10% LT MAD</span> are shown in the table below; sites below <span style = "color: #F90010;">5% MAD</span> are highlighted. The report shows conditions as of today.
...
if (params$table_format == "html") {
  ...
  leaflet() %>%
   ...
   fitBounds(48, 50, -139, -120) %>%
   setView( lng = -126, lat = 54.2, zoom = 5) %>%
   addMapPane("Drought Basins", zIndex = 400) %>% # order so the ploygons are above the drought monitor. Lower number puts the layer closer to the bottom
   addMapPane("Drought Polygons", zIndex = 410) %>%
...
### Table of Stations: 7 Day Maximum Daily Temperature above 18degC

The table below shows stations with a 7-day maximum daily temperature (i.e., the max daily temperature from the past 7 days) above <span style="colour:#B31F23;">18degC</span>. Also shown are the days from the past week in which the maximum daily temperature exceeded either <span style="colour:#B31F23;">18degC</span> or <span style="colour:#F90010;">23degC</span> (critical thresholds for fish health). If no sites are present, all sites with water temperature data show 7-day maximum daily temperatures below 18degC. 
...
if (params$table_format == "html"){
  watertemp %>%
    dplyr::mutate(`Mean Max Daily temp (last 7 days; deg C)` = cell_spec(`Mean Max Daily temp (last 7 days; deg C)`, "html", 
                                                                     color = ifelse(`Mean Max Daily temp (last 7 days; deg C)`  >= 18, "#B31F23", "#000000"))) %>%
...
These statistics include:
  ...
  - **Mean Max Daily temp (last 7 days; deg C)**: Mean of the max daily temperatures from the past week. Temperatures greater than 18 degrees C can indicate increased stress on fish populations.
  - **Max temp over last 24 hours (degC)**: Maximum temperature over the last 24 hours (degree C).
  - **Dates above 23 degC and 18 degC in last 7 days**: Dates from the past week that the stream temperature has gone beyond either 18 or 23 degrees C.
  ...
  table_all %>%
                                                                         ...
                                                                         ifelse(`Percentile Category - Last 24 Hour Q` == "Normal (25-75)" , "#08FA09", 
                                                                                ifelse(`Percentile Category - Last 24 Hour Q` == "Above normal (76-90)" , "#46DED2", 
                                                                                       ifelse(`Percentile Category - Last 24 Hour Q` == "Much above normal (>90)" , "#0003F6",
                                                                                              "#000000")))))))) %>%
    dplyr::mutate(`Percentile Category - 7 Day Q` = cell_spec(`Percentile Category - 7 Day Q`, "html",
                                            color = ifelse(is.na(`Percentile Category - 7 Day Q`), "#000000",
                                              ifelse(`Percentile Category - 7 Day Q`  == "Low", "#F90010",
                                                           ifelse(`Percentile Category - 7 Day Q` == "Much below normal (<10)", "#B31F23" ,
                                                                  ifelse(`Percentile Category - 7 Day Q` == "Below Normal (10-24)" , "#FEA116", 
                                                                         ifelse(`Percentile Category - 7 Day Q` == "Normal (25-75)" , "#08FA09", 
                                                                                ifelse(`Percentile Category - 7 Day Q` == "Above normal (76-90)" , "#46DED2", 
    ...
    dplyr::mutate(`Mean Max Daily temp (last 7 days; deg C)` = cell_spec(`Mean Max Daily temp (last 7 days; deg C)`, "html", 
                                                                     color = ifelse(is.na(`Mean Max Daily temp (last 7 days; deg C)`), "#000000",
                                                                       ifelse(`Mean Max Daily temp (last 7 days; deg C)`  >= 18, "#F90010", "#0003F6")))) %>%
    dplyr::mutate(`Max temp over last 24 hours (degC)` = cell_spec(`Max temp over last 24 hours (degC)`, "html", 
