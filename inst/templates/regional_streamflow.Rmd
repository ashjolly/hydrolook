...
if (params$table_format == "html") {
 ...
 # pal <- colorFactor(palette = naww_pal, domain = expected)
  ...
  leaflet() %>%
   ...
   addMapPane("polygons", zIndex = 410) %>% # order so the polygons are above the drought monitor. Lower number puts the layer closer to the bottom
   addMapPane("Drought Polygons", zIndex = 400) %>%
   addMapPane("Streamflowpoints", zIndex = 410) %>%
   ...
   addPolygons(data = region_watershed_interactive,
              ...
              weight = 3,
              color = '#696969',
              fillColor = '#696969', fillOpacity = 0.1, # completely opaque drought polygons for now
              highlightOptions = highlightOptions(color = "black", weight = 3,
                                                  bringToFront = FALSE),
...
if (params$table_format == "html") {
 ...
 # pal <- colorFactor(palette = naww_pal, domain = expected)
  ...
  leaflet() %>%
   ...
   fitBounds(48, 50, -139, -120) %>%
   setView( lng = -126, lat = 54.2, zoom = 5 ) %>%
   addMapPane("polygons", zIndex = 410) %>% # order so the ploygons are above the drought monitor. Lower number puts the layer closer to the bottom
   addMapPane("CDM", zIndex = 400) %>%
   addMapPane("Streamflowpoints", zIndex = 410) %>%
   addLayersControl(baseGroups = c("Drought Polygons", 
                                   "All sites - % MAD",
                                   "Sites with % MAD < 10%"),
...
if (params$table_format == "html") {
  ...
  naww_pal_temp <- c("#FFFFFF", 
                     ...
                     "#FC4E2A",
                     "#B10026") #Dark red
  temp_levels <- c("Not ranked", "<16 degC", "16 to 17", "17 to 18", "18 to 19", "19 to 20", "21 to 22", "22 to 23", "> or equal to 23")
  ...
  leaflet() %>%
   ...
   addMapPane("Streamflowpoints", zIndex = 410) %>%
   addLayersControl(baseGroups = c("Drought Polygons",
                                   "All sites - 7 Day Mean Max Daily Temp",
