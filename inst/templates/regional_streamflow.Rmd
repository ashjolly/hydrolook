...
pct_flow_last24 <- calc_percentiles(historical_flow = hist_flow, realtime_data = rl_data_last24)
...
#record <- tidyhydat::hy_stn_data_range() %>%
#  dplyr::filter(DATA_TYPE == "T")

#test <- realtime_dd(station_number = "08MF005")

#unique(test$Parameter)
token = token_ws()
...
extra_variables <- full_join(Q7, temp_station)
...
extra_variables <- full_join(extra_variables, hist_flow_7day_mean_day)

table_out <- full_join(data.frame(pct_flow_instant_tbl_data), extra_variables) %>%
  dplyr::select(-geometry, -X.tile.7day_mean, -date_dmy) %>%
  dplyr::arrange(X.tile.last24) %>%
  dplyr::rename(`Last 24 hour Q (m3/s)` = Latest.Q, 
                `Latest 7 Day Q (m3/s)` = Q7_value,
                `Percentile - Last 24 Hour Q (m3/s)` = X.tile.last24,
                `ID` = STATION_NUMBER,
...
much_below_normal_tbl <- pct_flow_instant %>%
  dplyr::filter(pct_bin %in% c("Low","Much below normal (<10)")) %>%
  #st_set_geometry(NULL) %>%
  dplyr::select(STATION_NAME, STATION_NUMBER, Value, prctile) %>%
  dplyr::rename(`Latest Q` = Value, `%tile` = prctile) %>%
  dplyr::arrange(STATION_NUMBER)
